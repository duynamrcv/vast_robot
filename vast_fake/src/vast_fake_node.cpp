...
void updateVastFake(tf::TransformBroadcaster &tf_broadcaster);
void updateOdometry(ros::Duration diff_time);
void updateTF(geometry_msgs::TransformStamped& odom_tf);

int main(int argc, char** argv)
{
    ros::init(argc, argv, "vast_fake_node");
    ros::NodeHandle nh("~");
    ...
    ros::Rate r(10);
    cout << "Initial Vast Robot Fake !!!" << endl;
    while(nh.ok())
    {
        updateVastFake(tf_broadcaster);
        ros::spinOnce();
        r.sleep();
    }
